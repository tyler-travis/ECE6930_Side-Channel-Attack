...
%File Parsing
numTracesPreAv = 199089;
numTraces = 9955;

%Load in Power Traces
traces = zeros(numTracesPreAv,1250);

for i = 1:numTracesPreAv 
    f=['../HiRes-recs/sample' strrep(num2str(zeros(1,5-length(num2str(i)))), ' ', '') num2str(i) '.mat'];
	load(f);
    traces(i,:) = .008*rec(1,:);
    disp(i);
end
...
traceAv = zeros(numTraces,1250);

for i = 1:20:numTracesPreAv
    if i == 199081
        traceAv(floor(i/20)+1,:) = mean(traces(i:i+8,:));
    else
...
left = uint32(hex2dec(dataArray{1,1}(1:9955)));
right = uint32(hex2dec(dataArray{1,2}(1:9955)));
...
fFunction_output = uint32(zeros(9955,64));
...
dbit = uint32(zeros(9955, 64));
% sets = uint32(zeros(9955, 64));
setB1 = uint32(zeros(9955, 64));
setB2 = uint32(zeros(9955, 64));
setB3 = uint32(zeros(9955, 64));
setB4 = uint32(zeros(9955, 64));
for sbox = 1:8
    for key = 1:64
